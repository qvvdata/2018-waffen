...
exports_sumat_all <- exports %>%
  filter(category =="c") %>%
  #filter(year!="2017") %>%
  filter(name_source_country == "Austria")

exports_for_export <- exports %>%
  left_join(sparte_schoen %>% select(wkdok,schön), by=c(wkdok="wkdok")) %>%
  mutate(wkdok = case_when(is.na(schön) ~ wkdok,
                           TRUE ~ schön)) %>%
  select(-schön)

exports_sumat <- exports_for_export %>%
  filter(category == "c") %>%
  filter(name_source_country == "Austria" & name_destination_country=="Uruguay")%>%
  group_by(wkdok)%>%
  summarise(mlsum= sum(value))

write_excel_csv(exports_sumat, "output/exports_sumat.csv")
write.csv(exports_sumat %>% rename(value=mlsum, name=wkdok), 'interaktiv/waffen-scroller/src/data/exports_sumat.csv', fileEncoding='utf-8', row.names = F)
...
```{r}
exports_destination_at <- exports_for_export %>%
  filter(category =="c") %>%
  ...
  filter(!is.na(iso_destination_country)) %>%
  filter(name_source_country == "Austria") %>%
  group_by(iso_destination_country, iso_destination_country_3, name_destination_country_de, wkdok,
           lat_source_country, long_source_country, lat_destination_country, long_destination_country) %>%
  summarise(mlsum= sum(value)) %>% arrange(-mlsum) %>% rename(name_destination_country="name_destination_country_de")


write_excel_csv(exports_destination_at, "output/exports_destination_at.csv")
write.csv(exports_destination_at, 'interaktiv/waffen-scroller/src/data/exports_countries.csv', fileEncoding='utf-8', row.names = F)
...
eiu <- eiu %>%
  mutate(class = ifelse(eiu>=8, "Vollständige Demokratien", 
                        ifelse(eiu>=6, "Unvollständige Demokratien", 
                               ifelse(eiu>=4, "Hybridregime", "Autoritäre Regime"))))

eiu$iso3 <- countrycode(eiu$GWn, "gwn", "iso3c") 
eiu$iso2 <- countrycode(eiu$GWn, "gwn", "iso2c") 
                               
# Wir müssen auf 2015 fixieren, weil sonst Länder von einer Kategorie in die nächste rutschen könnten
exports_democracy <- exports %>%
  filter(year>2014, iso_source_country=="AT") %>%
  left_join(eiu %>% filter(year=="2015"), by=c("iso_destination_country"="iso2")) 
  
  
exports_democracy_sumat <- exports_democracy %>%
  filter(category =="c") %>%
  mutate( class = if_else(is.na(class), "unbewertet", class), 
          name_destination_country_german = if_else(name_destination_country_german=="Russische Föderation", "Russland", name_destination_country_german)) %>%
  #filter(year!="2017") %>%
  filter(name_source_country == "Austria") %>%
  group_by(class, name_destination_country_german) %>%
  summarise(mlsum= sum(value)) %>%
  arrange(desc(mlsum))%>%
  na.omit()

exports_democracy_sumat_all <- exports_democracy %>%
  filter(category =="c") %>%
  mutate( class = if_else(is.na(class), "unbewertet", class), 
          name_destination_country_german = if_else(name_destination_country_german=="Russische Föderation", "Russland", name_destination_country_german)) %>%
  #filter(year!="2017") %>%
...
write.xlsx(as.data.frame(exports_democracy_sumat), "output/ignore/exports_democracy_sumat_countries.xlsx")

# Welche Waren an autoritäre Regime?
exports_democracy_sumat_all <- exports_democracy %>%
  filter(category =="c") %>%
  mutate( class = if_else(is.na(class), "unbewertet", class), 
          name_destination_country_german = if_else(name_destination_country_german=="Russische Föderation", "Russland", name_destination_country_german)) %>%
  #filter(year!="2017") %>%
  filter(name_source_country == "Austria" & class=="Autoritäre Regime") %>%
  group_by(class, eudok) %>%
  summarise(mlsum= sum(value)) %>%
  arrange(desc(mlsum))%>%
  na.omit()

# Wie viele Denials gibt es?
export_denials_austria <- exports %>%
  filter(category =="d" & name_source_country=="Austria") 
...
exports_sumat_yemen_dw_sa_vae <- exports %>%
  filter(category =="c") %>%
  filter(year>="2004") %>% # Konflikt beginnt im März 2015
  filter(name_destination_country == "Saudi Arabia" | name_destination_country == "United Arab Emirates") %>%
  filter(name_source_country == "Austria") %>%
  group_by(name_source_country, name_destination_country, year) %>%
  summarise(mlsum= sum(value)) %>%
  spread(name_destination_country, mlsum) %>%
  mutate(number_pretty = as.character(prettyNum(mlsum, big.mark =".")), 
         number_pretty_mio = round(mlsum/1000000,2))
  
write.xlsx(as.data.frame(exports_sumat_yemen_dw_sa_vae), "output/ignore/exports_sumat_yemen_dw_sa_vae.xlsx")
